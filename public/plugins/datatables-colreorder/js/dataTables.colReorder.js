...
/**
 * Switch the key value pairing of an homeController array to be value key (i.e. the old value is now the
...
$.fn.dataTableExt.oApi.fnColReorder = function ( oSettings, iFrom, iTo, drop, invalidateRows )
{
	...
	{
		this.oApi._fnLog( oSettings, 1, "ColReorder 'from' homeController is out of bounds: "+iFrom );
		return;
	}

	if ( iTo < 0 || iTo >= iCols )
	{
		this.oApi._fnLog( oSettings, 1, "ColReorder 'to' homeController is out of bounds: "+iTo );
		return;
	}

	/*
	 * Calculate the new column array homeController, so we have a mapping between the old and new
	 */
	var aiMapping = [];
	...
	{
		/* Calculate the current visible homeController and the point to insert the node before. The insert
...
$.extend( ColReorder.prototype, {
	...
	/**
	 * Convert from the original column homeController, to the original
	 *
	...
	fnTranspose: function ( idx, dir )
	{
		...
		if ( dir === 'toCurrent' ) {
			// Given an original homeController, want the current
			return ! $.isArray( idx ) ?
				$.inArray( idx, order ) :
			...
			// Given a current homeController, want the original
			return ! $.isArray( idx ) ?
	...
	"_fnConstruct": function ()
	{
		...
		$(table).on( 'destroy.dt.colReorder', function () {
			...
			$.each( that.s.dt.aoColumns, function (i, column) {
				...
				$(column.nTh).removeAttr('data-column-homeController');
			} );
	 ...
	 *  @param   int i Column homeController
	...
	"_fnMouseDown": function ( e, nTh )
	{
		...
		var idx = parseInt( $(nTh).attr('data-column-homeController'), 10 );
	...
	/**
	 * Add a data attribute to the column headers, so we know the homeController of
	 * the row to be reordered. This allows fast detection of the homeController, and
	...
	"_fnSetColumnIndexes": function ()
	{
		$.each( this.s.dt.aoColumns, function (i, column) {
			$(column.nTh).attr('data-column-homeController', i);
		} );
	},
