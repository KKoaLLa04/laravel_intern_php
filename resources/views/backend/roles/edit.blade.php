...
@section('content')
    ...
    <hr>
    <form action="{{route('admin.roles.post_edit', $roleDetail)}}" method="POST" enctype="multipart/form-data">
        @csrf
        @if(!empty($errors->any()))
        ...
        <div class="row">
            <div class="col-12">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" placeholder="Tên chức vụ...." name="name" value="{{old('name') ?? $roleDetail->name}}">
                    @error('name')
            ...
            <div class="col-12">
                <div class="form-group">
                    ...
                    <textarea class="form-control" name="display_name" placeholder="Mô tả chức vụ...">{{old('display_name') ?? $roleDetail->display_name}}</textarea>
                    @error('display_name')
                    <span style="color: red">{{$message}}</span>
                    @enderror
                </div>
                            ...
                            <td>
                                <div class="row">
                                    @foreach($permissionParentItem->permissionChildren as $permissionChildrentItem)
                                        <div class="col-2">
                                            <input type="checkbox" value="{{$permissionChildrentItem->id}}" name="permission_id[]" id="permission_{{$permissionParentItem->name}}_{{$permissionChildrentItem->name}}" {{$permissionChecked->contains('id', $permissionChildrentItem->id) ? 'checked':false}}>
